...
class PDFReportGenerator:
    """
    MZ-15 Enterprise-Grade PDF Intelligence Generator.
    ...
    def generate_company_pdf(self, company: str, data: dict) -> bytes:
        ...
        buf = io.BytesIO(); doc = self._create_doc(buf)
        elements = [
            Paragraph(f"💼 CompanyIntel Corporate OSINT Dossier: {company}", title_style),
            Paragraph(f"Industry: <b>{data.get('industry', 'Enterprise Tech')}</b> | HQ: <b>{data.get('headquarters', 'Global')}</b> | Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}", sub_style),
            Paragraph("Corporate Overview & Profile", h2_style),
            Paragraph(data.get("extract", "No extract found.")[:1200], body_style),
            Spacer(1, 10),
            Paragraph("Workplace Environment, Food & Culture", h2_style)
        ]
        cult_table = [
            [Paragraph("<b>Work-Life Balance</b>", bold_style), Paragraph("<b>Learning & Growth</b>", bold_style), Paragraph("<b>Food & Cafeteria</b>", bold_style), Paragraph("<b>RTO Policy</b>", bold_style)],
            [Paragraph(f"{data.get('wlb_score', 4.0)} / 5.0", title_style), Paragraph(f"{data.get('learning_score', 4.5)} / 5.0", title_style), Paragraph(f"{data.get('food_score', 4.5)} / 5.0", title_style), Paragraph(data.get('rto_policy', 'Hybrid')[:30], body_style)]
        ]
        tc = Table(cult_table, colWidths=[135, 135, 135, 135])
        tc.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#f8fafc")), ('GRID', (0,0), (-1,-1), 1, colors.HexColor("#cbd5e1")), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('PADDING', (0,0), (-1,-1), 6)]))
        elements.extend([tc, Spacer(1, 10)])

        # Salary Matrix
        salaries = data.get("salaries", [])
        if salaries:
            elements.append(Paragraph("Compensation Forensics (USD & INR)", h2_style))
            s_table = [[Paragraph("<b>Role / Level</b>", bold_style), Paragraph("<b>US Market ($)</b>", bold_style), Paragraph("<b>India Market (₹)</b>", bold_style)]]
            for s in salaries[:4]:
                s_table.append([Paragraph(s.get("role", "")[:35], body_style), Paragraph(s.get("us_range", ""), body_style), Paragraph(s.get("in_range", ""), body_style)])
            ts = Table(s_table, colWidths=[200, 170, 170])
            ts.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor("#0284c7")), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#e2e8f0")), ('PADDING', (0,0), (-1,-1), 5)]))
            elements.extend([ts, Spacer(1, 10)])

        # Drama & Gossip Radar
        drama = data.get("drama_radar", {})
        if drama.get("items"):
            elements.append(Paragraph("Monitored Discussion & Controversy Wire", h2_style))
            for item in drama.get("items", [])[:3]:
                elements.append(Paragraph(f"• <b>[{item.get('tag')}]</b> {item.get('headline')} (<i>{item.get('source')}</i>)", body_style))
            elements.append(Spacer(1, 10))

        # Financial Health & P&L
        fin = data.get("financials", {})
        if fin:
            elements.append(Paragraph("Financial Health, Profit & Loss Overview", h2_style))
            elements.append(Paragraph(f"<b>Annual Revenue:</b> {fin.get('annual_revenue', 'N/A')} | <b>Net Profit:</b> {fin.get('net_profit', 'N/A')} | <b>Growth:</b> {fin.get('yoy_growth', 'N/A')}", body_style))
            elements.append(Paragraph(f"<b>Cash Reserves:</b> {fin.get('cash_reserves', 'N/A')} | <b>Balance Sheet Health:</b> {fin.get('health_status', 'N/A')}", body_style))
            elements.append(Paragraph(f"<b>Runway & Layoff Safety Verdict:</b> {fin.get('runway_verdict', 'N/A')}", body_style))
            elements.append(Spacer(1, 10))

        # Recent Acquisitions
        acqs = data.get("acquisitions", [])
        if acqs:
            elements.append(Paragraph("Recent Acquisitions & M&A Deals", h2_style))
            for a in acqs[:3]:
                elements.append(Paragraph(f"• <b>{a.get('company')}</b> ({a.get('deal_value')}, {a.get('year')}) — {a.get('rationale')} [<i>{a.get('status')}</i>]", body_style))
            elements.append(Spacer(1, 10))

        # Employee Survival Briefing
        surv = data.get("survival_briefing", {})
        if surv:
            elements.append(Paragraph("Employee Survival Briefing", h2_style))
            elements.append(Paragraph(f"• <b>Org DNA:</b> {surv.get('org_dna', 'N/A')}", body_style))
            elements.append(Paragraph(f"• <b>Promotion Strategy:</b> {surv.get('promotion_secrets', 'N/A')}", body_style))
            elements.append(Paragraph(f"• <b>Layoff Safety:</b> {surv.get('pip_layoff_safety', 'N/A')} | <b>Notice Period:</b> {surv.get('notice_period', 'N/A')}", body_style))
            elements.append(Spacer(1, 10))
    ...
    def generate_agency_seo_audit_pdf(self, domain: str, data: dict, agency_name: str = "MZ-15 Intelligence Suite") -> bytes:
        res = self._get_base_styles()
        if not res: return b""
    ...
    def generate_instagram_pdf(self, profile: dict) -> bytes:
        ...
        # 5 Collab Suggestions Table
        collabs = profile.get("collab_suggestions", [])
        if collabs:
            elements.append(Paragraph("5 High-Synergy Creator Collaboration Matches", h2_style))
            collab_rows = [[
                Paragraph("<b>Creator / Handle</b>", bold_style),
                Paragraph("<b>Audience Tier</b>", bold_style),
                Paragraph("<b>Synergy Match</b>", bold_style),
                Paragraph("<b>Collaboration Strategy</b>", bold_style)
            ]]
            for c in collabs[:5]:
                collab_rows.append([
                    Paragraph(f"<b>{c.get('name')}</b><br/><font color='#0284c7'>@{c.get('handle')}</font>", body_style),
                    Paragraph(c.get("tier", ""), body_style),
                    Paragraph(f"<b><font color='#16a34a'>{c.get('match_score')}%</font></b>", bold_style),
                    Paragraph(f"{c.get('strategy')}<br/><font color='#64748b'>Format: {c.get('format')}</font>", body_style)
                ])
            t_collab = Table(collab_rows, colWidths=[140, 100, 90, 210])
            ...
            elements.append(t_collab)

        doc.build(elements)
        buf.seek(0)
        return buf.getvalue()

    # 25. YOUTUBE MASTER (100+ Point Channel & Algorithm Audit)
    def generate_youtube_pdf(self, yt_data: dict) -> bytes:
        ...
        doc = self._create_doc(buf)
        elements = [
            Paragraph(f"📺 YouTubeMaster Intelligence: {yt_data.get('channel_title', 'Channel')} (@{yt_data.get('handle', '')})", title_style),
            Paragraph(
                f"Generated: <b>{datetime.now().strftime('%Y-%m-%d %H:%M')}</b> | "
                f"Niche: <b>{yt_data.get('niche', 'General')}</b> | "
                f"Country: <b>{yt_data.get('country', 'Global')}</b> | "
                f"Overall Score: <b><font color='#dc2626'>{yt_data.get('overall_score', 0)}% ({yt_data.get('overall_grade', 'N/A')})</font></b>",
                sub_style
            ),
            Paragraph("Executive Channel Authority & Diagnostics", h2_style)
        ]

        # Key Metrics Table
        m_rows = [
            [Paragraph("<b>Metric</b>", bold_style), Paragraph("<b>Value</b>", bold_style), Paragraph("<b>Status / Benchmark</b>", bold_style)],
            [Paragraph("Subscribers", body_style), Paragraph(f"<b>{yt_data.get('subscribers_str', 'N/A')}</b>", bold_style), Paragraph("Public Surface Extraction", body_style)],
            [Paragraph("Total Lifetime Views", body_style), Paragraph(f"<b>{yt_data.get('views_str', 'N/A')}</b>", bold_style), Paragraph("Algorithmic Velocity Base", body_style)],
            [Paragraph("Published Videos", body_style), Paragraph(f"<b>{yt_data.get('videos_str', 'N/A')}</b>", bold_style), Paragraph("Catalog Library Depth", body_style)],
            [Paragraph("Total Audit Checks", body_style), Paragraph(f"<b>{yt_data.get('passed_checks', 0)} / {yt_data.get('total_checks', 100)} Passed</b>", bold_style), Paragraph(f"Grade: {yt_data.get('overall_grade', 'N/A')}", bold_style)],
        ]
        t_m = Table(m_rows, colWidths=[180, 160, 200])
        ...
        # View Traffic Analytics Table
        traffic = yt_data.get("view_traffic", {})
        if traffic:
            elements.append(Paragraph("Real-Time View Traffic & Velocity Forensics", h2_style))
            vt_rows = [
                [Paragraph("<b>Timeframe / Metric</b>", bold_style), Paragraph("<b>Estimated Views</b>", bold_style), Paragraph("<b>Algorithmic Benchmark</b>", bold_style)],
                [Paragraph("7-Day Views (Weekly Run-Rate)", body_style), Paragraph(f"<b>{traffic.get('views_7d_str', 'N/A')}</b>", bold_style), Paragraph(f"{traffic.get('views_7d', 0):,} views", body_style)],
                [Paragraph("30-Day Views (Monthly Volume)", body_style), Paragraph(f"<b>{traffic.get('views_30d_str', 'N/A')}</b>", bold_style), Paragraph(f"{traffic.get('views_30d', 0):,} views", body_style)],
                [Paragraph("Average Daily Views", body_style), Paragraph(f"<b>{traffic.get('daily_views_str', 'N/A')}</b>", bold_style), Paragraph("Daily audience velocity", body_style)],
                [Paragraph("Hourly Velocity (VPH)", body_style), Paragraph(f"<b>{traffic.get('vph_str', 'N/A')}</b>", bold_style), Paragraph("Views-Per-Hour pacing", body_style)],
                [Paragraph("Views-to-Subscriber Liquidity", body_style), Paragraph(f"<b>{traffic.get('liquidity_ratio', 0)}x</b>", bold_style), Paragraph(traffic.get('momentum_status', 'Active')[:30], body_style)],
            ]
            t_vt = Table(vt_rows, colWidths=[180, 160, 200])
            ...
            elements.append(t_vt)
            elements.append(Spacer(1, 10))

        # Commercial Rate Card
        rates = yt_data.get("commercial_rates", {})
        if rates:
            elements.append(Paragraph("Commercial Sponsorship Valuation Rate Card", h2_style))
            rate_rows = [
                [Paragraph("<b>Asset Format</b>", bold_style), Paragraph("<b>Projected Rate (USD)</b>", bold_style), Paragraph("<b>Market Basis</b>", bold_style)],
                [Paragraph("Dedicated Video Feature", body_style), Paragraph(f"<b>${rates.get('dedicated_video_low', 0):,} - ${rates.get('dedicated_video_high', 0):,}</b>", bold_style), Paragraph("Per sponsored video", body_style)],
                [Paragraph("60-Second Mid-Roll Integration", body_style), Paragraph(f"<b>${rates.get('midroll_integration', 0):,}</b>", bold_style), Paragraph("Standard agency placement", body_style)],
                [Paragraph("Vertical Short Placement", body_style), Paragraph(f"<b>${rates.get('short_integration', 0):,}</b>", bold_style), Paragraph("Per sponsored short", body_style)],
                [Paragraph("Est. Annual Deal Capacity", body_style), Paragraph(f"<b>${rates.get('annual_deal_potential', 0):,}</b>", bold_style), Paragraph("Annual projected revenue", body_style)]
            ]
            t_rates = Table(rate_rows, colWidths=[180, 180, 180])
            t_rates.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#0f172a")),
                ('TEXTCOLOR', (0,0), (-1,0), colors.white),
                ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#cbd5e1")),
                ('PADDING', (0,0), (-1,-1), 4)
            ]))
            elements.append(t_rates)
            elements.append(Spacer(1, 10))

        # 7-Pillar Breakdown
        categories = yt_data.get("categories", {})
        if categories:
            elements.append(Paragraph("7-Pillar Algorithmic Audit Breakdown", h2_style))
            cat_rows = [[Paragraph("<b>Pillar</b>", bold_style), Paragraph("<b>Score</b>", bold_style), Paragraph("<b>Checks Passed</b>", bold_style), Paragraph("<b>Grade</b>", bold_style)]]
            for c_name, c_data in categories.items():
                cat_rows.append([
                    Paragraph(c_name, bold_style),
                    Paragraph(f"{c_data.get('pct', 0)}%", body_style),
        ...
        # 5 Collab Suggestions Table
        collabs = yt_data.get("collab_suggestions", [])
        if collabs:
            elements.append(Paragraph("5 High-Synergy Creator Collaboration Matches", h2_style))
            collab_rows = [[
                Paragraph("<b>Creator / Handle</b>", bold_style),
                Paragraph("<b>Audience</b>", bold_style),
                Paragraph("<b>Niche Match</b>", bold_style),
                Paragraph("<b>Strategy & Production Format</b>", bold_style)
            ]]
            for c in collabs[:5]:
                collab_rows.append([
                    Paragraph(f"<b>{c.get('name')}</b><br/><font color='#dc2626'>@{c.get('handle')}</font>", body_style),
                    Paragraph(f"{c.get('subscribers')} Subs", body_style),
                    Paragraph(f"<b><font color='#16a34a'>{c.get('match_score')}%</font></b>", bold_style),
                    Paragraph(f"{c.get('strategy')}<br/><font color='#64748b'>Format: {c.get('format')}</font>", body_style)
                ])
            t_collab = Table(collab_rows, colWidths=[140, 90, 80, 230])
            t_collab.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#1e293b")),
                ('TEXTCOLOR', (0,0), (-1,0), colors.white),
                ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#cbd5e1")),
        ...
        # Top Priority Recommendations
        recs = yt_data.get("recommendations", [])
        if recs:
            elements.append(Paragraph("High-Priority Actionable Roadmap", h2_style))
            r_rows = [[Paragraph("<b>Priority</b>", bold_style), Paragraph("<b>Area</b>", bold_style), Paragraph("<b>Action Item</b>", bold_style)]]
            for r in recs[:5]:
                r_rows.append([
                    Paragraph(f"<b>{r.get('priority', 'MED')}</b>", bold_style),
    ...
    # 26. JOB RADAR (Fast Naukri & LinkedIn Approval Dossier)
    def generate_job_radar_pdf(self, job_data: dict) -> bytes:
        ...
        elements = [
            ...
            Paragraph(
                f"Generated: <b>{datetime.now().strftime('%Y-%m-%d %H:%M')}</b> | "
                f"Location: <b>{job_data.get('location', 'India')}</b> | "
                f"Fast-Approval (<24h): <b><font color='#16a34a'>{job_data.get('fast_approval_count', 0)} of {job_data.get('total_found', 0)} Openings</font></b>",
                sub_style
            ),
            Paragraph("Top Fast-Track Openings (Ranked by Response Velocity)", h2_style)
        ]

        # Top Jobs Table
        j_rows = [[
            Paragraph("<b>Role & Company</b>", bold_style),
            Paragraph("<b>Area Micro-Hub</b>", bold_style),
            Paragraph("<b>Est. Salary</b>", bold_style),
            Paragraph("<b>Approval Velocity</b>", bold_style)
        ]]
        for j in job_data.get("jobs", [])[:8]:
            j_rows.append([
                Paragraph(f"<b>{j.get('title')}</b><br/><font color='#0284c7'>{j.get('company')}</font> ({j.get('platform')})", body_style),
                Paragraph(j.get("area_hub", ""), body_style),
                Paragraph(j.get("salary", "Competitive"), body_style),
                Paragraph(f"<b><font color='#16a34a'>{j.get('velocity_score')}/100</font></b><br/><font color='#64748b'>{j.get('speed_badge')}</font>", body_style)
            ])
        t_jobs = Table(j_rows, colWidths=[180, 150, 110, 100])
        ...
        elements.append(t_jobs)
        elements.append(Spacer(1, 10))

        # Area Distribution Table
        area_dist = job_data.get("area_distribution", {})
        if area_dist:
            elements.append(Paragraph("Area Micro-Hub Hiring Density", h2_style))
            a_rows = [[Paragraph("<b>Micro-Tech Zone / Park</b>", bold_style), Paragraph("<b>Active Fast Openings</b>", bold_style)]]
            for a_name, count in sorted(area_dist.items(), key=lambda x: x[1], reverse=True)[:6]:
                a_rows.append([Paragraph(a_name, body_style), Paragraph(f"<b>{count} openings</b>", bold_style)])
            t_area = Table(a_rows, colWidths=[380, 160])
        ...
        if pitch:
            elements.append(Paragraph("Direct Recruiter InMail Outreach Script", h2_style))
            elements.append(Paragraph(f"<i>Optimal timing: {pitch.get('best_time_to_apply')}</i>", sub_style))
            p_text = pitch.get("linkedin_inmail", "").replace("\n", "<br/>")
            t_pitch = Table([[Paragraph(p_text, body_style)]], colWidths=[540])
    ...
    # 27. CLIENT HUNTER (Agency B2B Client Acquisition & Audit Dossier)
    def generate_client_hunter_pdf(self, client_data: dict) -> bytes:
        ...
        doc = self._create_doc(buf)
        elements = [
            Paragraph(f"🎯 ClientHunter Proposal: {client_data.get('niche', 'Local Business')} Audit", title_style),
            Paragraph(
                f"Generated: <b>{datetime.now().strftime('%Y-%m-%d %H:%M')}</b> | "
                f"Target Locality: <b>{client_data.get('location', 'India')}</b> | "
                f"Hot Prospects: <b><font color='#dc2626'>{client_data.get('hot_leads_count', 0)} of {client_data.get('total_leads_found', 0)} Leads</font></b>",
                sub_style
            ),
            Paragraph("High-Priority Client Audit Roster", h2_style)
        ]

        # Table of Audited Leads
        l_rows = [[
            Paragraph("<b>Business Name</b>", bold_style),
            Paragraph("<b>Digital Gaps (Website/GBP/SEO)</b>", bold_style),
            Paragraph("<b>Est. Deal Value</b>", bold_style),
            Paragraph("<b>Lead Priority</b>", bold_style)
        ]]
        for l in client_data.get("leads", [])[:6]:
            l_rows.append([
                Paragraph(f"<b>{l.get('business_name')}</b><br/><font color='#64748b'>{l.get('location')}</font>", body_style),
                Paragraph(f"• Web: {l.get('web_status')[:30]}<br/>• GBP: {l.get('gbp_status')[:30]}", body_style),
                Paragraph(f"<b><font color='#16a34a'>{l.get('est_deal_value')}</font></b>", bold_style),
                Paragraph(f"<b>{l.get('lead_priority')[:25]}</b>", bold_style)
            ])
        t_leads = Table(l_rows, colWidths=[140, 200, 110, 90])
        ...
        # GBP Keywords Table
        kws = client_data.get("gbp_keywords", [])
        if kws:
            elements.append(Paragraph("High-Intent Locality Google Business Profile (GBP) Keywords", h2_style))
            k_rows = [[Paragraph("<b>Search Term</b>", bold_style), Paragraph("<b>Intent</b>", bold_style), Paragraph("<b>Search Volume</b>", bold_style)]]
            for k in kws[:6]:
                k_rows.append([
                    Paragraph(k.get("keyword", ""), body_style),
                    Paragraph(k.get("intent", ""), body_style),
                    Paragraph(k.get("search_vol", ""), bold_style)
                ])
            t_kw = Table(k_rows, colWidths=[260, 160, 120])
        ...
        # Client WhatsApp Pitch
        pitches = client_data.get("pitches", {})
        if pitches:
            elements.append(Paragraph("High-Converting WhatsApp Client Acquisition Pitch", h2_style))
            p_text = pitches.get("whatsapp_dm", "").replace("\n", "<br/>")
            t_pitch = Table([[Paragraph(p_text, body_style)]], colWidths=[540])
            t_pitch.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#f1f5f9")),
                ('BOX', (0,0), (-1,-1), 1, colors.HexColor("#94a3b8")),
